  ...
  return (
    <Layout className="" dark={true}>
      ...
      <div className="container pt-20 mb-10 mx-auto text-center">
        ...
        <div className=" p-10">
            <p className="text-black mb-5">While I am not a professional photographer, I do enjoy taking my camera around the neighborhood.</p>
            <p className="text-black mb-5">Here is my modest collection of photos, which I hope to add to soon.</p>
          </div>
      </div>
      <div className="container mx-auto grid md:grid-cols-2 gap-4 px-5 md:px-0 pb-20">
          
          {
            photos.map((photo) => <div key={`photo-${photo.node.id}`} className="relative ">
                <GatsbyImage image={photo.node.Photo.localFile.childImageSharp.gatsbyImageData} className="w-full transition duration-500 hover:scale-110 z-10 transform"/>
